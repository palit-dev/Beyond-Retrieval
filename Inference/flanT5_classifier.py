...
class FlanT5Dataset(Dataset):
    """
    Custom PyTorch dataset for handling data required for FlanT5 model testing.
...
class Test():
    """
    Class for testing the FlanT5 model.
    ...
    def __init__(self, model_dir: str, test_df_path: str, output_path: str) -> None:
        ...
        self.df_test = FlanT5Dataset(self.TEST_DF_PATH, tokenizer = self.tokenizer)
    ...
    def test(self):
        ...
        for batch in tqdm(self.loader_test, total = len(self.loader_test), desc = 'Samples'):
            ...
            predictions['predicted_output'].extend(batch_scores.argmax(-1).tolist())
...
if __name__ == '__main__':
    ...
    logging.info('Initiating Flan T5 Classifier Testing....')
    t.test()
    logging.info('Flan T5 Classifier Testing completed')