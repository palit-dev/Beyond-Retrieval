...
class T5Dataset(Dataset):
    """
    Custom PyTorch dataset for handling data required for T5 model training.
    ...
    Example:
    >>> dataset = T5Dataset('path/to/data.pkl', tokenizer, is_train=True)
    >>> len(dataset)
    1000
    ...
    def __getitem__(self, idx):
        ...
        text = f"Topic: {sample['topic']} Title: {sample['bib_title']} Paragraph: {sample['paragraph']} Relevant:"
...
class Train():
     """
    Class for configuring and training the T5 model.
    ...
    Example:
    >>> trainer = Train('cache_dir', 'train.pkl', 'val.pkl', 'output_dir')
    >>> trainer.train()
    """
    ...
    def __init__(self, cache_dir, df_train_path, df_val_path, output_dir):
        ...
        self.BATCH_SIZE = 3
        self.GRADIENT_ACCUMULATION_STEP = 60
        ...
        # Model and Tokenizer
        self.model = AutoModelForSeq2SeqLM.from_pretrained(self.BASE_MODEL_NAME, cache_dir = self.CACHE_DIR)
        self.tokenizer = AutoTokenizer.from_pretrained(self.BASE_MODEL_NAME, cache_dir = self.CACHE_DIR)

        # Load Train and Test Dataframes
        self.df_train = T5Dataset(self.TRAIN_DF_PATH, tokenizer = self.tokenizer, is_train = True)
        self.df_val = T5Dataset(self.VALIDATION_DF_PATH, tokenizer = self.tokenizer, is_train = False)
...
if __name__ == "__main__":
    ...
    train_df_path = os.path.join(dir_path, os.path.normpath('Datasets/df_train_retriever.pkl'))
    val_df_path = os.path.join(dir_path, os.path.normpath('Datasets/df_val_retriever.pkl'))
    output_dir = os.path.join(dir_path, os.path.normpath('T5_Output'))
    ...
    logging.info('Initiating T5 Retriever Training')
    t.train()
    logging.info('T5 Retriever Training completed')